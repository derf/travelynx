...
sub get {
	...
	for my $entry ( $res->expand->hashes->each ) {
		...
		my $ref = {
			...
			distance_beeline  => $entry->{distance_beeline},
			distance_route    => $entry->{distance_route},
			distance_polyline => $entry->{distance_polyline},
			distance => $entry->{distance_polyline} // $entry->{distance_route}
			  // $entry->{distance_beeline},
